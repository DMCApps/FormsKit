...
struct FormPersistenceUserDefaultsTests {
    ...
    // MARK: - FormSynchronousPersistence (loadSynchronously)

    @Test("loadSynchronously returns saved values")
    func loadSynchronouslyReturnsSavedValues() async throws {
        let suiteName = makeSuiteName()
        let defaults = UserDefaults(suiteName: suiteName)!
        defer { defaults.removePersistentDomain(forName: suiteName) }

        let persistence = FormPersistenceUserDefaults(defaults: defaults)
        try await persistence.save(makeStore(["theme": .string("dark")]), formId: "ui")

        let loaded = persistence.loadSynchronously(formId: "ui")
        #expect(loaded["theme"] == .string("dark"))
    }

    @Test("loadSynchronously returns empty store when nothing saved")
    func loadSynchronouslyReturnsEmptyWhenNotSaved() {
        let suiteName = makeSuiteName()
        let defaults = UserDefaults(suiteName: suiteName)!
        defer { defaults.removePersistentDomain(forName: suiteName) }

        let persistence = FormPersistenceUserDefaults(defaults: defaults)
        let loaded = persistence.loadSynchronously(formId: "nonexistent")
        #expect(loaded.isEmpty == true)
    }

    @Test("loadSynchronously respects key prefix")
    func loadSynchronouslyRespectsKeyPrefix() async throws {
        let suiteName = makeSuiteName()
        let defaults = UserDefaults(suiteName: suiteName)!
        defer { defaults.removePersistentDomain(forName: suiteName) }

        let p1 = FormPersistenceUserDefaults(defaults: defaults, keyPrefix: "app1")
        let p2 = FormPersistenceUserDefaults(defaults: defaults, keyPrefix: "app2")

        try await p1.save(makeStore(["x": .int(10)]), formId: "cfg")
        try await p2.save(makeStore(["x": .int(20)]), formId: "cfg")

        let loaded1 = p1.loadSynchronously(formId: "cfg")
        let loaded2 = p2.loadSynchronously(formId: "cfg")

        #expect(loaded1["x"] == .int(10))
        #expect(loaded2["x"] == .int(20))
    }

    @Test("loadSynchronously returns empty after clear")
    func loadSynchronouslyEmptyAfterClear() async throws {
        let suiteName = makeSuiteName()
        let defaults = UserDefaults(suiteName: suiteName)!
        defer { defaults.removePersistentDomain(forName: suiteName) }

        let persistence = FormPersistenceUserDefaults(defaults: defaults)
        try await persistence.save(makeStore(["key": .bool(true)]), formId: "form")
        try await persistence.clear(formId: "form")

        let loaded = persistence.loadSynchronously(formId: "form")
        #expect(loaded.isEmpty == true)
    }

    @Test("FormPersistenceUserDefaults conforms to FormSynchronousPersistence")
    func conformsToSynchronousPersistence() {
        let persistence = FormPersistenceUserDefaults()
        // Compile-time check: if this compiles, the conformance is correct.
        let _: any FormSynchronousPersistence = persistence
        #expect(Bool(true)) // reached means conformance check passed
    }
}
